...
namespace NexusOps.Web.Pages.Areas
{
    ...
    public class IndexModel : NxPageModel
    {
        ...
        public List<AreaDto> Items { get; set; } = new();
        ...
        public async Task OnGetAsync(string? buscar)
        {
            ...
            var r = await _api.GetAsync<List<AreaDto>>(ApiService.Url("api/areas", ("buscar", buscar)));
        ...
        {
            var actual = await _api.GetAsync<AreaDto>($"api/areas/{id}");
            ...
            var cuerpo = new AreaDto { IdArea = x.IdArea, Nombre = x.Nombre, Descripcion = x.Descripcion, Activo = nuevoEstado };

            var r = await _api.PutAsync<AreaDto>($"api/areas/{id}", cuerpo);
